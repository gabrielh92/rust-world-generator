...
use crate::{canvas::CanvasData, mathlib::vec::Vec2, params::ParamGroup, pipeline::{points::PointsOutput, PipelineStage, PipelineStageExecutor, StageData, StageDataMap}, util::make_stage_data_key, visualization::voronoi::VoronoiVisualLayer};
...
impl PipelineStageExecutor for VoronoiStage {
	...
	fn run(&mut self, _params: Option<&ParamGroup>, data: &StageDataMap) -> Box<dyn StageData> {
		...
		let points = data.get(make_stage_data_key("points", 1).as_str())
            .and_then(|d| d.as_any().downcast_ref::<PointsOutput>())
            .expect("Points stage must run before Voronoi stage");

        let input_points = points.points.iter().map(|p| voronoice::Point {x: p.x as f64, y: p.y as f64}).collect();
		...
		diagram.iter_cells().for_each(|cell| {
			let vertices: Vec<Vec2> = cell.iter_vertices()
